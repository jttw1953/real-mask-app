...
const { deleteMeeting } = await import('../../src/supabase_api/supabase_api.js');
...
describe('DELETE /api/delete-meeting/:id', () => {
    ...
    describe('Validation Tests - Meeting ID', () => {
        ...
        test('should accept negative meeting ID', async () => {
            deleteMeeting.mockResolvedValue(null);
            const response = await request(app)
                .delete('/api/delete-meeting/-1')
                .set('Authorization', validAuthToken)
                .expect(200);
        ...
        test('should accept very large meeting ID', async () => {
            ...
            const response = await request(app)
                ...
                .expect(200);
            expect(response.body.message).toBe('Meeting deleted successfully');
            expect(deleteMeeting).toHaveBeenCalledWith(parseInt(largeId, 10), mockUserId);
        });
        /**
    ...
    describe('Error Cases from Database', () => {
        ...
        test('should return 400 when deleteMeeting returns error', async () => {
            deleteMeeting.mockResolvedValue('Meeting not found');
            const response = await request(app)
        ...
        test('should return 400 when user not authorized to delete meeting', async () => {
            deleteMeeting.mockResolvedValue('Not authorized to delete this meeting');
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(400);
        ...
        test('should return 500 for unexpected errors', async () => {
            deleteMeeting.mockRejectedValue(new Error('Unexpected database error'));
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(500);
        ...
        test('should return 500 when network error occurs', async () => {
            deleteMeeting.mockRejectedValue(new Error('Network timeout'));
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(500);
        ...
        test('should return 500 when database connection fails', async () => {
            deleteMeeting.mockRejectedValue(new Error('Database connection failed'));
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(500);
        ...
        test('should return 400 for constraint violation', async () => {
            deleteMeeting.mockResolvedValue('Cannot delete meeting with active participants');
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(400);
            expect(response.body.error).toBe('Cannot delete meeting with active participants');
        });
    });
    ...
    describe('Edge Cases', () => {
        ...
        test('should handle deletion of already deleted meeting', async () => {
            deleteMeeting.mockResolvedValue('Meeting already deleted');
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(400);
    ...
    describe('Response Format Tests', () => {
        ...
        test('should return correct response structure on error', async () => {
            deleteMeeting.mockResolvedValue('Some error message');
            const response = await request(app)
                .delete('/api/delete-meeting/123')
                .set('Authorization', validAuthToken)
                .expect(400);
...
//# sourceMappingURL=delete_meeting.test.js.map