...
const { getAllMeetings } = await import('../../src/supabase_api/supabase_api.js');
...
describe('GET /api/get-all-meetings', () => {
    ...
    describe('Success Cases - Meeting Data Variations', () => {
        ...
        test('should handle meetings with special characters', async () => {
            ...
            const response = await request(app)
                ...
                .set('Authorization', validAuthToken)
                .expect(200);
            expect(response.body.meetings[0].meeting_title).toBe('Q&A Session: Year-End Review (2025)');
        });
        /**
        ...
        test('should handle meetings with different timezone formats', async () => {
            ...
            const response = await request(app)
                ...
                .expect(200);
            expect(response.body.meetings).toHaveLength(3);
            expect(response.body.meetings[0].meeting_time).toBe('2025-12-01T10:00:00Z');
            expect(response.body.meetings[1].meeting_time).toBe('2025-12-01T10:00:00-08:00');
        });
        /**
    ...
    describe('Error Cases from Database', () => {
        ...
        test('should return 500 for unexpected errors', async () => {
            getAllMeetings.mockRejectedValue(new Error('Unexpected database error'));
            const response = await request(app)
                .get('/api/get-all-meetings')
                .set('Authorization', validAuthToken)
                .expect(500);
        ...
        test('should return 500 when network error occurs', async () => {
            getAllMeetings.mockRejectedValue(new Error('Network timeout'));
            const response = await request(app)
                .get('/api/get-all-meetings')
                .set('Authorization', validAuthToken)
                .expect(500);
    ...
    describe('Edge Cases', () => {
        ...
        test('should call getAllMeetings with only userId', async () => {
            ...
            expect(getAllMeetings).toHaveBeenCalledWith(mockUserId);
            expect(getAllMeetings).toHaveBeenCalledTimes(1);
            expect(getAllMeetings.mock.calls[0]).toHaveLength(1);
        });
        /**
...
//# sourceMappingURL=get_all_meetings.test.js.map