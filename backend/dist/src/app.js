...
import fs from 'fs';
import { createUser, getUserOverlays, deleteOverlay, uploadOverlay, scheduleMeeting, deleteMeeting, getAllMeetings, updateUserFullName, deleteUser, updateMeeting, supabase, getUserData, } from './supabase_api/supabase_api.js';
...
app.get('/api/get-all-overlays', async (req, res) => {
    const { userId, error: authError } = await verifyAuthToken(req.headers.authorization);
    if (authError || !userId) {
...
app.post('/api/schedule-meeting', async (req, res) => {
    ...
    try {
        ...
        const result = await scheduleMeeting(userId, meeting_code, meeting_time, meeting_title);
        if (result.error) {
            res.status(400).json({ error: result.error.message });
            return;
        }
...
app.put('/api/update-meeting/:id', async (req, res) => {
    ...
    try {
        ...
        const error = await updateMeeting(meetingIdNum, userId, meeting_title, meeting_time, meeting_code);
        if (error) {
            res.status(400).json({ error: error });
            return;
        }
