...
async function getOverlayImage(overlayUrl) {
...
export function bufferToCanvas(frameData, width, height) {
 ...
 */
export async function processFrame(canvas, overlayUrl, opacity) {
    const ctx = canvas.getContext('2d');
...
export function clearOverlayCache(overlayUrl) {
    if (overlayUrl) {
 ...
 */
export function startFrameProcessing(rtpPort, producerId, rtpParameters, onFrameProcessed) {
    console.log(`🎬 Starting FFmpeg decoder on port ${rtpPort}`);
    const videoCodec = rtpParameters.codecs.find((c) => c.mimeType.startsWith('video/'));
 ...
 */
export async function startFrameEncoding(outputRtpPort, producerId, rtpParameters, width, height) {
