...
app.put('/api/update-user-name', async (req, res) => {
  ...
  try {
    ...
    // Check for undefined or null FIRST (but not other falsy values)
...
app.put('/api/update-meeting/:id', async (req, res) => {
  ...
  try {
    ...
    // Validate that the meeting ID contains only digits (and optional leading minus sign)
    if (!/^-?\d+$/.test(meetingId)) {
      res.status(400).json({ error: 'Invalid meeting ID format' });
      return;
    }
