...
const toBuffer = (stored: string): Buffer =>
  Buffer.from(JSON.parse(stored).data);
...
export async function getUserOverlays(ownerId: string): Promise<{
  ...
  if (signErr) return { data: [], error: signErr };
...
export async function deleteOverlay(
  ...
  if (!row) return 'Overlay not found';

  const { error: rmErr } = await supabase.storage
    .from('overlays')
    .remove([row.storage_path]);

  if (rmErr && (rmErr as any).status !== 404) return rmErr.message;
...
export async function scheduleMeeting(
  ...
  meetingTime: string,
  meetingTitle: string
): Promise<{ data: { id: number } | null; error: any }> {
  try {
...
export async function getUserData(
  ...
  try {
    ...
    const decrypted = (data as any[]).map((row) => ({
      id: row.id,
...
export async function deleteUser(userId: string): Promise<undefined | string> {
  try {
