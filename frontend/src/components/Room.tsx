...
import { Mic, MicOff, Video, VideoOff, User, MessageCircle } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import type { Overlay } from '../types/overlayType';
import { DEFAULT_OVERLAY } from '../types/overlayType';
import OverlaySelector from './OverlaySelector';
import ChatPanel from './ChatPanel';
import { generateKeyPair, decryptMessage, base64ToUint8Array, uint8ArrayToBase64 } from '../utils/encryption';
...
export const Room = ({
    ...
    const [remotePublicKey, setRemotePublicKey] = useState<Uint8Array | null>(null);
    ...
    // Initialize encryption key pair on mount
    useEffect(() => {
        const keyPair = generateKeyPair();
        setLocalKeyPair(keyPair);
        console.log('🔑 Generated local key pair for E2E encryption');
    }, []);
    ...
    useEffect(() => {
        ...
        socket.on('partner-connected', ({ meetingId: connectedMeetingId }) => {
            ...
            // Send public key for E2E encryption
            if (localKeyPair) {
                const publicKeyBase64 = uint8ArrayToBase64(localKeyPair.publicKey);
                socket.emit('public-key', { publicKey: publicKeyBase64 });
                console.log('🔑 Sent local public key to partner');
            }
        });
        ...
        // NEW: Handle key exchange for E2E encryption
        socket.on('public-key', ({ publicKey }: { publicKey: string }) => {
            console.log('🔑 Received remote public key');
            try {
                const remoteKey = base64ToUint8Array(publicKey);
                setRemotePublicKey(remoteKey);
                console.log('✅ Remote public key set for encryption');
            } catch (error) {
        ...
        socket.on('encrypted-chat-message', async (encrypted: EncryptedMessage) => {
            if (!localKeyPair) {
                console.error('❌ No local key pair for decryption');
                return;
            }

            try {
                const senderPublicKey = base64ToUint8Array(encrypted.publicKey);
                const decrypted = decryptMessage(encrypted, senderPublicKey, localKeyPair.secretKey);
                console.log('🔓 Decrypted message:', decrypted);
                if ((window as any).addChatMessage) {
                    (window as any).addChatMessage(decrypted);
                }
            } catch (error) {
                console.error('❌ Failed to decrypt message:', error);
                // Fallback: show encrypted indicator
                if ((window as any).addChatMessage) {
                    (window as any).addChatMessage('[Encrypted message - decryption failed]');
                }
            }
        });
        ...
        <div className="h-screen flex flex-col overflow-hidden bg-black">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span className="text-white text-[9px] mt-0.5">Chat</span>
                        </button>
            ...
            <ChatPanel
                ...
                onSendMessage={(message) => {
                    if (socket) {
                        // If message is an encrypted object, send it via encrypted-chat-message
                        if (typeof message === 'object' && 'ciphertext' in message) {
                            socket.emit('encrypted-chat-message', {
                                meetingId,
                                encrypted: message
                            });
                        } else {
                            // Otherwise send as plain text (fallback)
