...
export const Room = ({
    ...
    // Mediasoup setup - replaces old P2P WebRTC
    useEffect(() => {
        if (!connected || !socket) {
            return;
        }

        console.log('🔧 Setting up Mediasoup connection...');
        ...
        const initializeMediasoup = async () => {
            try {
                ...
                // Step 2: Request router capabilities from server
                socket.emit('get-router-capabilities');
                
                socket.once('router-capabilities', async (routerRtpCapabilities: any) => {
                    if (cleanup) return;
                    ...
                    if (params.direction === 'send') {
                        ...
                        transport.on('connect', async ({ dtlsParameters }, callback, errback) => {
                            try {
                                ...
                                const connectHandler = () => {
                                    callback();
                                    socket.off('transport-connected', connectHandler);
                                };
                                
                                socket.on('transport-connected', connectHandler);
                            } catch (error) {
                        ...
                        transport.on('produce', async ({ kind, rtpParameters }, callback, errback) => {
                            try {
                                ...
                                socket.once('producer-created', ({ id }: { id: string }) => {
                                    callback({ id });
                                });
                            } catch (error) {
                                errback(error as Error);
                ...
                socket.on('new-producer', async ({ producerId, kind }: { producerId: string, kind: string }) => {
                    ...
                    console.log('📥 New producer available:', kind);
                    ...
                    console.log('✅ Consuming producer immediately:', kind);
                    socket.emit('consume', {
                ...
                socket.on('consumer-created', async (params: any) => {
                    if (cleanup) return;
                    
                    console.log('📥 Consuming', params.kind);
                    ...
                    if (params.kind === 'video') {
                        ...
                        if (remoteVideoRef.current) {
                            ...
                            console.log('✅ Remote video track set and playing');
                        }
                    }
    ...
    const drawFaceOverlay = (
        ...
        const faceWidth = Math.abs((rightPoint.x - leftPoint.x) * canvas.width);
        const faceHeight = Math.abs((bottomPoint.y - topPoint.y) * canvas.height);
        
        const centerX = ((leftPoint.x + rightPoint.x) / 2) * canvas.width;
        const centerY = ((topPoint.y + bottomPoint.y) / 2) * canvas.height;
        
        const overlayWidth = faceWidth * 1.5;
        const overlayHeight = faceHeight * 1.5;
